...
def main(args):
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer, step_size=args.n_epochs // 5, gamma=args.gamma
    )
    ...
    for epoch in range(args.n_epochs):
        ...
        if valid_score.mean() > best_valid_score or (
            args.early_stop < 0 and epoch % 4 == 0
        ):
            print(
                "\tvalid metric (%s) updates from %.6f to %.6f"
